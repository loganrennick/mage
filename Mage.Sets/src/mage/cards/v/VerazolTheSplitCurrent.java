...
public final class VerazolTheSplitCurrent extends CardImpl {
    ...
    public VerazolTheSplitCurrent(UUID ownerId, CardSetInfo setInfo) {
        ...
        effect.setText("with a +1/+1 counter on it for each mana spent to cast it.");
        ...
        effect = new CopyTargetSpellEffect(true);
        FilterSpell filter = StaticFilters.FILTER_SPELL_KICKED_A;
        this.addAbility(new SpellCastControllerTriggeredAbility(new DoIfCostPaid(effect, new RemoveCountersSourceCost(CounterType.P1P1.createInstance(2))), filter, false, true));
    }
